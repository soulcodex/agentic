...
BLOCK
  ...
  if [[ -f "$gitignore" ]] && grep -qF "# agentic:start" "$gitignore" 2>/dev/null; then
    has_block=true
  fi

  if [[ -f "$gitignore" && "$has_block" == "true" ]]; then
    # Replace existing managed block in-place using pure bash parameter expansion
    local full before after
    full=$(< "$gitignore")
    before="${full%%# agentic:start*}"
    after="${full##*# agentic:end}"
    printf '%s%s%s' "$before" "$AGENTIC_BLOCK" "$after" > "$gitignore"
