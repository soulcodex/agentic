...
assert_file_contains_block() {
  local file="$1" pattern="$2" label="$3"
  local content
  content=$(cat "$file" 2>/dev/null || true)
  if [[ "$content" == *"$pattern"* ]]; then
    pass "$label — block present"
  else
    fail "$label — missing block in $file"
  fi
}
...
assert_line_count() {
  local file="$1" line="$2" expected="$3" label="$4"
  local count
  count=$(grep -cxF -- "$line" "$file" 2>/dev/null || true)
  if [[ "$count" -eq "$expected" ]]; then
    pass "$label — line count $count matches: $line"
  else
    fail "$label — expected line count $expected for '$line', got $count"
  fi
}
...
assert_line_count "$TMP/t_gl/.gitignore" ".agentic/skills" 1 "T_GITIGNORE_LINK_MODE"
assert_line_count "$TMP/t_gl/.gitignore" ".agentic/fragments" 1 "T_GITIGNORE_LINK_MODE"
assert_line_count "$TMP/t_gl/.gitignore" ".agentic/vendor-files" 1 "T_GITIGNORE_LINK_MODE"
assert_line_count "$TMP/t_gl/.gitignore" ".agentic/agents" 1 "T_GITIGNORE_LINK_MODE"
...
EOF
bash "$COMPOSE" --library "$LIBRARY" --profile golang-hexagonal-cobra-cli \
  --target "$TMP/t_gst" --link > /dev/null 2>&1
assert_line_count "$TMP/t_gst/.gitignore" ".agentic/skills" 1 "T_GITIGNORE_STRIP_STALE_LINK_TAIL"
assert_line_count "$TMP/t_gst/.gitignore" ".agentic/fragments" 1 "T_GITIGNORE_STRIP_STALE_LINK_TAIL"
assert_line_count "$TMP/t_gst/.gitignore" ".agentic/vendor-files" 1 "T_GITIGNORE_STRIP_STALE_LINK_TAIL"
assert_line_count "$TMP/t_gst/.gitignore" ".agentic/agents" 1 "T_GITIGNORE_STRIP_STALE_LINK_TAIL"
assert_grep_count "$TMP/t_gst/.gitignore" "# agentic:start — managed block (do not edit manually, regenerated by agentic)" 1 "T_GITIGNORE_STRIP_STALE_LINK_TAIL"
assert_grep_count "$TMP/t_gst/.gitignore" "# agentic:end" 1 "T_GITIGNORE_STRIP_STALE_LINK_TAIL"
...
assert_file_contains_block "$TMP/t74/AGENTS.md" $'<!-- local override: AGENTS.local.md -->\n\n## Custom Rules' "T74 spacing"
...
# T76A — compose: markdown section boundaries keep blank-line separators
run_test "T76A — compose: markdown section spacing is normalized"
mkdir -p "$TMP/t76a"
bash "$COMPOSE" \
  --library "$LIBRARY" \
...
assert_file_contains "$TMP/t99/.agentic/config.yaml" "deploy_mode: copy" "T99 deploy mode copy"
...
# T100A — init --link: config sets deploy_mode: link
run_test "T100A — init --link sets deploy_mode: link"
mkdir -p "$TMP/t100a"
AGENTIC_REPO_ROOT="$LIBRARY" "$CLI" init "$TMP/t100a" --link --no-sync > /dev/null 2>&1
assert_file_contains "$TMP/t100a/.agentic/config.yaml" "deploy_mode: link" "T100A deploy mode link"
...
# T101A — init --link --sync: composes in link mode
run_test "T101A — init --link --sync composes with fragments symlink"
mkdir -p "$TMP/t101a"
...
bash "$COMPOSE" \
  ...
  --target "$TMP/t101b" \
  > /dev/null 2>&1
if [[ -L "$TMP/t101b/.agentic/fragments" ]]; then
  fail "T101B precondition: fragments should start as copied directory"
fi
rm -rf "$TMP/t101b/.agentic/fragments"
cat > "$TMP/t101b/.agentic/config.yaml" <<EOF
